<template>
  <div class="pt100" @keydown="onKeydown">
    <div class="h550 w400 flex flex-column flex-items-center pl35 pr35 bg-white_1 bd center ">
      ...
      <my-button primary big class="fit-width" @click="loginHandle" :disabled='!userName||!password||loading'><MyLoadIcon v-if="loading" nontext /><span v-else>登录</span></my-button>
...
export default {
  data: function(){
    return { userName: 'asd',  password: 'dsadsads', userNameError: '', passwordError: '', loading: false}
  },
  methods: {
    ...
    loginHandle: function() {
      ...
      if(this.passwordError || this.userNameError) return this.loading = false
      this.$store.dispatch('user/login',{count:this.userName, password:this.password}).finally(()=>{
        this.loading = false
      })
    },
    ...
    onKeydown: function(e) {
      ...
      key == 13 &&(this.userName && this.password && !this.loading && !this.dialogopen) && (this.loginHandle()) 
    }
  },
}
</script>
